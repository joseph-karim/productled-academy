...
Model Types:
1. Freemium
   - Core features free forever, premium behind paywall
   - Best for products with clear feature tiers
   - Requires strong free/paid value balance
...
3. Open Core
   - Basic features open source, enterprise features paid
   - Best for developer tools and platforms
      ...
      functions: [
        {
          ...
          parameters: {
            ...
            properties: {
              model: {
                type: "string",
                enum: ["freemium", "free-trial", "open-core", "community"]
              },
              confidence: {
                type: "number",
                minimum: 0,
  ...
  return handleOpenAIRequest(
    openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an expert product strategist. Given a product description and desired user outcome, suggest potential challenges that users might face.

For each challenge:
1. Provide a clear, concise title
2. Include a detailed description
3. Assess magnitude (1-5) based on:
   - Impact on user success
   - Frequency of occurrence
   - Difficulty to overcome
4. Include reasoning for the assessment

Focus on realistic challenges that:
- Directly impact the user's ability to achieve their desired outcome
- Are specific and actionable
- Represent common user pain points
- Can be addressed through product features or improvements`
        },
        {
          role: "user",
          content: `
...
${level.charAt(0).toUpperCase() + level.slice(1)} User Outcome: ${outcome}

Generate 2-3 challenges that ${level} users might face when trying to achieve this outcome.`
        }
      ],
      functions: [
        {
          name: "suggest_challenges",
          description: "Suggest challenges for users",
          parameters: {
            type: "object",
            properties: {
              suggestions: {
                ...
                items: {
                  ...
                  properties: {
                    title: { type: "string" },
                    description: { type: "string" },
                    magnitude: { type: "number", minimum: 1, maximum: 5 },
                    reasoning: { type: "string" }
                  },
                  required: ["title", "description", "magnitude", "reasoning"]
                }
              }
            },
            required: ["suggestions"]
